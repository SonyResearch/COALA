...
def broadcast_model(model, src=0):
    """Broadcast updated model to different gpus.

    Args:
        model (nn.Module): The model in a device to aggregate.
        src (int): The source gpu.
    """
...
def broadcast_model_dict(model_dict, src=0):
    """Broadcast updated model to different gpus.

    Args:
        model_dict (dict (nn.Module)): The model in a device to aggregate.
        src (int): The source gpu.
    """
...
def reduce_models(model, sample_sum):
    ...
    if sample_sum <= 0:
        logger.debug("Sum of total dataset sizes is 0.")
        return
    state = model.state_dict()
    for k in state.keys():
