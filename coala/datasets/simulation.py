...
def iid(data_x, data_y, num_of_clients, x_dtype, y_dtype, seed=0):
...
def non_iid_dirichlet(data_x, data_y, num_of_clients, alpha, min_size, x_dtype, y_dtype, seed=0):
...
def non_iid_class(data_x, data_y, class_per_client, num_of_clients, x_dtype, y_dtype, stack_x=True, seed=0):
...
def data_simulation(data_x, data_y, num_of_clients, data_distribution, weights=None, alpha=0.5, min_size=10,
                    class_per_client=1, stack_x=True, seed=0):
    ...
    if data_distribution == SIMULATE_IID:
        group_client_list, group_federated_data = iid(data_x, data_y, num_of_clients, x_dtype, y_dtype, seed)
        ...
        group_client_list, group_federated_data = non_iid_class(data_x, data_y, class_per_client, num_of_clients,
                                                                x_dtype,
                                                                y_dtype, stack_x=stack_x, seed=seed)
    else:
...
def split_labeled_and_unlabeled(data_x, data_y, semi_scenario, num_labels_per_class, num_of_clients):
    if semi_scenario == 'label_in_server':
        num_s = num_labels_per_class
    if semi_scenario == 'label_in_client':
