...
class FedSSLClient(BaseClient):
    ...
    def set_model(self, server_model):
    ...
    def update_model(self):
        if self.conf.model in [model.MoCo, model.MoCoV2]:
            self.model.encoder_q = self.server_model.encoder_q
        elif self.conf.model == model.SimCLR:
            self.model.online_encoder = self.server_model.online_encoder
        elif self.conf.model == model.SimSiam:
            if self._local_model is None:
                self.model.online_encoder = self.server_model.online_encoder
                self.model.online_predictor = self.server_model.online_predictor
                return

            if self.conf.update_encoder == ONLINE:
                online_encoder = self.server_model.online_encoder
            else:
            ...
            if self.conf.update_predictor == GLOBAL:
                predictor = self.server_model.online_predictor
            else:
        ...
        elif self.conf.model == model.BYOL:

            if self._local_model is None:
                logger.debug("Use aggregated encoder and predictor")
                self.model.online_encoder = self.server_model.online_encoder
                self.model.target_encoder = self.server_model.online_encoder
                self.model.online_predictor = self.server_model.online_predictor
                return

            def ema_online():
                self._calculate_weight_scaler()
                logger.debug(f"Encoder: update online with EMA of global encoder @ round {self.conf.round_id}")
            ...
            def ema_predictor():
                ...
                if distance > 0.5:
                    ...
                    model.update_moving_average(ema_updater, predictor, self.server_model.online_predictor)
                else:
                    weight = 1 - distance
                    ema_updater = model.EMA(weight)
                    predictor = self.server_model.online_predictor
            ...
            if self.conf.aggregate_encoder == ONLINE and self.conf.update_encoder == ONLINE:
                logger.debug("Encoder: aggregate online, update online")
                online_encoder = self.server_model.online_encoder
                target_encoder = self._local_model.target_encoder
            elif self.conf.aggregate_encoder == TARGET and self.conf.update_encoder == ONLINE:
                logger.debug("Encoder: aggregate target, update online")
                online_encoder = self.server_model.target_encoder
                target_encoder = self._local_model.target_encoder
            elif self.conf.aggregate_encoder == TARGET and self.conf.update_encoder == TARGET:
                logger.debug("Encoder: aggregate target, update target")
                online_encoder = self._local_model.online_encoder
                target_encoder = self.server_model.target_encoder
            elif self.conf.aggregate_encoder == ONLINE and self.conf.update_encoder == TARGET:
                logger.debug("Encoder: aggregate online, update target")
                online_encoder = self._local_model.online_encoder
                target_encoder = self.server_model.online_encoder
            elif self.conf.aggregate_encoder == ONLINE and self.conf.update_encoder == BOTH:
                logger.debug("Encoder: aggregate online, update both")
                online_encoder = self.server_model.online_encoder
                target_encoder = self.server_model.online_encoder
            elif self.conf.aggregate_encoder == TARGET and self.conf.update_encoder == BOTH:
                logger.debug("Encoder: aggregate target, update both")
                online_encoder = self.server_model.target_encoder
                target_encoder = self.server_model.target_encoder
            elif self.conf.update_encoder == NONE:
                logger.debug("Encoder: use local online and target encoders")
                online_encoder = self._local_model.online_encoder
                target_encoder = self._local_model.target_encoder
            elif self.conf.update_encoder == EMA:
                logger.debug(f"Encoder: use EMA, weight {self.conf.encoder_weight}")
                online_encoder = self._local_model.online_encoder
                ema_updater = model.EMA(self.conf.encoder_weight)
                model.update_moving_average(ema_updater, online_encoder, self.server_model.online_encoder)
                target_encoder = self._local_model.target_encoder
                ...
                if self.previous_trained_round + 1 == self.conf.round_id:
                    ...
                    logger.debug(f"Encoder: update online and target @ round {self.conf.round_id}")
                    online_encoder = self.server_model.online_encoder
                    target_encoder = self.server_model.online_encoder
            else:
                raise ValueError(f"Encoder: aggregate {self.conf.aggregate_encoder}, "
            ...
            if self.conf.update_predictor == GLOBAL:
                logger.debug("Predictor: use global predictor")
                predictor = self.server_model.online_predictor
            elif self.conf.update_predictor == LOCAL:
                logger.debug("Predictor: use local predictor")
                predictor = self._local_model.online_predictor
            elif self.conf.update_predictor == DAPU:
                # Divergence-aware predictor update (DAPU)
                logger.debug(f"Predictor: use DAPU, mu {self.conf.dapu_threshold}")
                ...
                logger.debug(f"Predictor: use EMA, weight {self.conf.predictor_weight}")
                predictor = self._local_model.online_predictor
    ...
    def train(self, conf, device=CPU):
        ...
        for i in range(conf.local_epoch):
            ...
            for (batched_x1, batched_x2), _ in self.train_loader:
                ...
                if conf.model == model.BYOL and conf.momentum_update:
        ...
        if conf.update_predictor in [DAPU, DYNAMIC_DAPU, SELECTIVE_EMA] or conf.update_encoder in [DYNAMIC_EMA_ONLINE,
                                                                                                   SELECTIVE_EMA]:
    ...
    def _calculate_divergence(self, old_model, new_model, typ=L2):
        ...
        logger.debug(f"Model distance: {distance} = {total_distance}/{size}")
        return distance
    ...
    def _calculate_weight_scaler(self):
        if not self.weight_scaler:
            ...
            logger.debug(f"Client {self.cid}: weight scaler {self.weight_scaler}")
    ...
    def load_optimizer(self, conf):
        ...
        # moco_v1 should use the default learning rate
        if conf.model == model.MoCo:
            lr = conf.optimizer.lr

        params = self.model.parameters()
        if conf.model == model.BYOL:
            params = [
    ...
    def post_upload(self):
        if self.conf.model == model.BYOL:
            del self.model
            del self.server_model
            self.model = None
            self.server_model = None
            assert self.model is None
            assert self.server_model is None
            gc.collect()
