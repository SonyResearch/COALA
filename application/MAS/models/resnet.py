...
class Decoder(nn.Module):
    def __init__(self, output_channels=32, num_classes=None, base_match=512, expansion=1):
        ...
        if tasks is not None:
            for task in tasks:
                ...
                if task == 'edge_texture':
                    output_channels = 1
                if size == 1:
                    decoder = Decoder(output_channels, expansion=block.expansion)
                elif size == 2:
                    decoder = Decoder(output_channels, base_match=720, expansion=block.expansion)
                elif size == 3:
                    decoder = Decoder(output_channels, base_match=880, expansion=block.expansion)
                elif size == 0.5:
                    decoder = Decoder(output_channels, base_match=360, expansion=block.expansion)
...
def resnet18_triple(pretrained=False, **kwargs):
    """Constructs a ResNet-18 model.
