...
from model import get_deeplab_network
from fl_client import SegClient
from fl_server import SegServer
from dataset.voc12 import construct_voc12_datasets
import coala
...
def run(args, rank=0):
    ...
    train_data, test_data = construct_voc12_datasets(conf.data.root,
                                                     conf.data.dataset,
                                                     conf.data.num_of_clients,
                                                     conf.data.split_type)
    # using pretrained model from pytorch
    model = get_deeplab_network(model=conf.model, backbone=conf.backbone, num_classes=conf.data.num_classes)

    # Initialize with the new config.
    coala.register_dataset(train_data, test_data)
    coala.register_model(model)
    coala.register_client(SegClient)
    coala.register_server(SegServer)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Fed_Segmentation')
    parser.add_argument("--task_id", type=int, default=0)
    parser.add_argument('--dataset', type=str, default='voc12', help="name of dataset, e.g., cifar10")
    ...
    parser.add_argument('--model', type=str, default="deeplabv3", help="seg model")
    ...
    parser.add_argument('--loss_type', type=str, default="cross_entropy", help='loss')
